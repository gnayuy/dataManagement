    ...
    pplx::task<void> requestTask = fstream::open_ostream(U("results.json")).then([=](ostream outFile)
                                                                                 {
                                                                                     *fileStream = outFile;
                                                                                     ...
                                                                                     uri_builder builder(U("/api/node/cd0/metadata/key/mouselightbranch3"));
                                                                                     return client.request(methods::GET, builder.to_string());
                                                                                 })
*/

    pplx::task<void> requestTask = fstream::open_ostream(U("keys.json")).then([=](ostream outFile)
                                                                                 {
                                                                                     *fileStream = outFile;
                                                                                     ...
                                                                                     // Build request URI and start the request.
                                                                                     uri_builder builder(U("/api/node/cd0/metadata/keys"));
                                                                                     return client.request(methods::GET, builder.to_string());
                                                                                 })
